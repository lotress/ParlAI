...
class ExampleSeq2seqAgent(TorchAgent):
    ...
    def __init__(self, opt, shared=None):
        ...
        if not shared:
            ...
            if 'optims' in states:
                for key in states['optims']:
                    self.optims[key].load_state_dict(states['optims'][key])
        elif 'encoder' in shared:
            # copy initialized data from shared table
            self.encoder = shared['encoder']
            self.decoder = shared['decoder']
            self.optims = shared['optims']
    ...
    def state_dict(self):
        ...
        states['model'] = tuple((m.module.state_dict() if hasattr(m, 'module') else m.state_dict()) for m in (self.encoder, self.decoder))
        states['optims'] = dict((key, self.optims[key].state_dict()) for key in self.optims)
        return states
